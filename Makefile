...
PYTHON?=python3
PYMAJ=$(shell $(PYTHON) -c 'import sys; print(sys.version_info.major)')
PYMIN=$(shell $(PYTHON) -c 'import sys; print(sys.version_info.minor)')

DEBUGFLAGS=-Og -ggdb -DDEBUG=1 -pg
OPTIMIZEFLAGS=-O3 -march=native -mtune=native
DISABLEDWARNINGS=-Wno-error=cast-function-type -Wno-error=unused-parameter
...
COMPILEFLAGS=${CPPFLAGS} ${CXXFLAGS} ${GENERALFLAGS} ${SECURITYFLAGS} ${DEBUGFLAGS}
...
PYTHONFLAGS=-I/usr/include/python${PYMAJ}.${PYMIN}m -lpython${PYMAJ}.${PYMIN}m
PYEXT=.cpython-${PYMAJ}${PYMIN}m-x86_64-linux-gnu.so
...
build/lib/libcmsh.so: build/.objects/constraint_t.o build/.objects/model_t.o
	$(CXX) $(WARNINGFLAGS) $(COMPILEFLAGS) -shared build/.objects/constraint_t.o build/.objects/model_t.o -o build/lib/libcmsh.so
