...
function rc_run() {
    ...
    if rc_buildah; then
        buildah bud --tag "cmsh_$image:latest" -f "tools/Dockerfiles/$image" .
        ret="$?"
        if [ "x$ret" != "x0" ]; then
        ...
        docker build --tag "cmsh_$image:latest" -f "tools/Dockerfiles/$image" .
        ret="$?"
        if [ "x$ret" != "x0" ]; then
            echo "Container build exited with status: $ret"
            return $ret
